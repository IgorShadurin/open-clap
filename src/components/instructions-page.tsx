...
export function InstructionsPage() {
  const [sets, setSets] = useState<InstructionSetTreeItem[]>([]);
  ...
  const [deleteTarget, setDeleteTarget] = useState<InstructionSetTreeItem | null>(null);
  ...
  useEffect(() => {
    const handleDocumentPointerDown = (event: MouseEvent) => {
      ...
      if (target instanceof Element && target.closest("[data-instruction-image-menu]")) {
        return;
      }
  ...
  const uploadSetImage = async (instructionSetId: string, file: File) => {
    ...
    const response = await fetch(`/api/instructions/${instructionSetId}/image`, {
      body: formData,
      method: "POST",
    });

    if (!response.ok) {
      throw new Error(`Instruction image upload failed: ${response.status}`);
    }
  };
  ...
  const handleCreate = async () => {
    ...
    setSubmitting(true);
    try {
      const created = await requestJson<InstructionSetEntity>("/api/instructions", {
        body: JSON.stringify({
      ...
      await loadSets();
      toast.success("Instruction set created");
    } catch (error) {
      setErrorMessage(error instanceof Error ? error.message : "Failed to create instruction set");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleCreateInstructionTask = async (
    ...
    try {
      ...
      setErrorMessage(error instanceof Error ? error.message : "Failed to create instruction task");
    }
  };

  const handleInstructionSetTasksToggle = async (set: InstructionSetTreeItem) => {
    const nextVisible = !set.mainPageTasksVisible;

    try {
      await requestJson<InstructionSetEntity>(`/api/instructions/${set.id}`, {
      ...
      setErrorMessage(
        error instanceof Error
          ? error.message
          : "Failed to update instruction task list visibility",
      );
    }
  };
  ...
  const handleSetImageUpload = async (instructionSetId: string, file: File) => {
    ...
    try {
      ...
      await loadSets({ silent: true });
      toast.success("Instruction set image updated");
    } catch (error) {
      setErrorMessage(error instanceof Error ? error.message : "Failed to upload instruction set image");
    } finally {
  ...
  const handleSetImageDelete = async (instructionSetId: string) => {
    ...
    try {
      await requestJson(`/api/instructions/${instructionSetId}/image`, { method: "DELETE" });
      bumpImageCacheBust(instructionSetId);
      setImageLoadErrors((previous) => {
        if (!previous[instructionSetId]) {
          return previous;
      ...
      setErrorMessage(error instanceof Error ? error.message : "Failed to delete instruction set image");
    } finally {
  ...
  const startSetNameEdit = (set: InstructionSetTreeItem) => {
  ...
  const saveSetNameEdit = async () => {
    ...
    try {
      await requestJson(`/api/instructions/${editingSetId}`, {
        body: JSON.stringify({
      ...
      await loadSets({ silent: true });
      cancelSetNameEdit();
      toast.success("Instruction set name updated");
    } catch (error) {
  ...
  const handleDelete = async () => {
    ...
    try {
      await requestJson(`/api/instructions/${deleteTarget.id}`, { method: "DELETE" });
      ...
      <div className="mx-auto w-full max-w-6xl space-y-6">
        ...
        {!loading
          ? sets.map((set) => {
              ...
              const imageSrc = `/api/instructions/${set.id}/image?v=${encodeURIComponent(imageVersion)}`;
                  ...
                  <CardContent className="space-y-3 py-4">
                    <div className="flex flex-wrap items-center justify-between gap-3">
                      <div className="flex min-w-0 items-center gap-3">
                        <div className="relative" data-instruction-image-menu>
                          <button
                            aria-expanded={openSetImageMenuId === set.id}
                            aria-haspopup="menu"
                            aria-label={`Instruction set image options for ${set.name}`}
                        ...
                        <div className="min-w-0">
                          {isSetNameEditing ? (
                            <div className="flex items-center gap-1">
                              ...
                              />
                              <Button
                                aria-label="Save instruction set name"
                              ...
                              <Button
                                aria-label="Cancel instruction set rename"
                                className="h-8 w-8 p-0"
                                disabled={editingSetSubmitting}
                            ...
                            <div className="group/instruction-name flex items-center gap-1">
                              <Link
                                className="truncate text-lg font-semibold underline-offset-4 hover:underline"
                                href={`/instructions/${set.id}`}
                                title={`Open ${set.name}`}
                              >
                                {set.name}
                              </Link>
                              <Button
                                aria-label={`Rename instruction set ${set.name}`}
                                className="h-7 w-7 p-0 opacity-0 transition-opacity group-hover/instruction-name:opacity-100"
                                onClick={() => startSetNameEdit(set)}
                                size="sm"
                                title={`Rename instruction set ${set.name}`}
                                type="button"
                                variant="ghost"
                              >
                    ...
                    <div className="space-y-2">
                      <div className="flex items-center justify-between gap-2">
                        ...
                        <Button
                          aria-label={setTasksVisible ? "Hide instruction tasks" : "Show instruction tasks"}
                          className="h-8 w-8 rounded-full p-0"
                          onClick={() => void handleInstructionSetTasksToggle(set)}
                          size="sm"
                          title={setTasksVisible ? "Hide instruction tasks" : "Show instruction tasks"}
                          type="button"
                          variant="outline"
                        >
                          {setTasksVisible ? <ArrowUp className="h-4 w-4" /> : <ArrowDown className="h-4 w-4" />}
                          <span className="sr-only">
                            {setTasksVisible ? "Hide instruction tasks" : "Show instruction tasks"}
                          </span>
                        </Button>
                      </div>
                      ...
                      {setTasksVisible ? (
                        <>
                          <TaskQuickAdd
                            ...
                            projectId={`instruction-set:${set.id}`}
        ...
        <Dialog onOpenChange={(open) => (!open ? setDeleteTarget(null) : undefined)} open={Boolean(deleteTarget)}>
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                Delete instruction set <strong>{deleteTarget?.name}</strong>? This action cannot be
                undone.
              </DialogDescription>
            </DialogHeader>
            <DialogFooter>
        ...
        >
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                Create a new instruction set and optionally attach an image.
              </DialogDescription>
            </DialogHeader>
